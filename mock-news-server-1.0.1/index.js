...
const HEADLINE_PATTERNS = [
  ...
  (s, a, o) => `${s} ${a} ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 2: Subject + "to" + Action + Object
  (s, a, o) => `${s} to ${a} ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 3: Object + "as" + Subject + Action
  (o, s, a) => `${o} as ${s} ${a} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 4: Subject + "could" + Action + Object
  (s, a, o) => `${s} could ${a} ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 5: Subject + "may" + Action + Object
  (s, a, o) => `${s} may ${a} ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 6: Subject + "seen" + Action + Object
  (s, a, o) => `${s} seen ${a} ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 7: Subject + "facing" + Object
  (s, _, o) => `${s} facing ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 8: Subject + "amid" + Object
  (s, _, o) => `${s} amid ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 9: Subject + "after" + Object
  (s, _, o) => `${s} after ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`,
  // Pattern 10: Subject + "despite" + Object
  (s, _, o) => `${s} despite ${o} ${lorem.generateWords(Math.floor(Math.random() * 7) + 1)}`
];

function generateRandomHeadlines() {
  try {
    const sources = ['Bloomberg', 'Reuters', 'Dow Jones', 'AP', 'People\'s Daily', 'Twitter', 'Truth Social', 'CNBC', 'SEC', 'WSJ', 'FT'];
    const subjects = ['US', 'China', 'Japan', 'Europe', 'EU', 'Russia', 'India', 'White House', 'Trump', 'Apple', 'Binance', 'Musk', 'Putin', 'Germany', 'France', 'Mexico', 'Canada'];
    const actions = ['warns', 'says', 'announces', 'expects', 'believes', 'blasts'];
    const objects = ['inflation', 'recession', 'trade war', 'COVID-19', 'economic recovery', ...subjects];
    ...
    const asset = Math.random() < 0.4 ? assets[Math.floor(Math.random() * assets.length)] : null;

    // Add link with 50% probability
    const link = Math.random() < 0.5 ? `https://${lorem.generateWords(Math.floor(Math.random() * 2) + 1).split(' ').join('-')}.com/${ulid()}` : null;

    const headlines = srcs.map(src => {
      const pattern = HEADLINE_PATTERNS[Math.floor(Math.random() * HEADLINE_PATTERNS.length)];

      // Add some random adjectives and adverbs
      const adjectives = ['Major', 'Significant', 'Unexpected', 'Surprising', 'Dramatic', 'Sudden', 'Gradual'];
      const adverbs = ['quickly', 'slowly', 'suddenly', 'gradually', 'significantly', 'dramatically'];
  ...
  try {
    ...
    if (shouldGenerate) {
      ...
      for (const headline of headlines) {
        ...
        try {
          const message = JSON.stringify({ ...headline, timestamp: new Date().valueOf() });
